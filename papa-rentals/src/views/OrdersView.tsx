    ...
    <div className="order-card">
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap' }}>
        {!done && !cancelled && (
          <button className="btn btn-outline btn-sm" onClick={() => { buzz(); dispatch({ type: 'ADVANCE_ORDER', orderId: order.id }) }}>
            <Icon name="skip" size={14} /> Skip ahead
          </button>
        )}
        {order.status === 'in_use' && (
          <button className="btn btn-outline btn-sm" onClick={() => setExtendOpen(true)}><Icon name="calendar" size={14} /> Extend rental</button>
        )}
        {cancellable && (
          <button className="btn btn-ghost btn-sm" onClick={() => setCancelOpen(true)}><Icon name="undo" size={14} /> Cancel order</button>
        )}
        {done && !order.myRatingOfOwner && (
          <button className="btn btn-primary btn-sm" onClick={() => setRateOpen(true)}><Icon name="star" size={14} /> Rate this rental</button>
        )}
        {done && order.myRatingOfOwner && (
          <span style={{ fontSize: 13, alignSelf: 'center' }}>
            You rated: <Stars value={order.myRatingOfOwner} size={12} /> · Owner rated you: <Stars value={order.ownerRatingOfMe ?? 5} size={12} /> <Badge tone="green">Published together <Icon name="check" size={14} /></Badge>
          </span>
        )}
        {(done || cancelled) && (
          <button className="btn btn-ghost btn-sm" onClick={bookAgain}><Icon name="repeat" size={14} /> Book again</button>
        )}
        {(done || order.status === 'returned' || order.status === 'in_use') && order.lines.some((l) => l.insurance) && !hasClaim && (
          <button className="btn btn-ghost btn-sm" onClick={() => setClaimOpen(true)}><Icon name="shield" size={14} /> File claim</button>
        )}
        <button className="btn btn-ghost btn-sm" onClick={() => go({ name: 'support' })}><Icon name="headset" size={14} /> Get help</button>
        <button className="btn btn-ghost btn-sm" onClick={() => downloadReceipt(order)}><Icon name="receipt" size={14} /> Receipt</button>
      ...
      {done && (
        <p className="small" style={{ color: 'var(--green)', fontWeight: 700, margin: '10px 0 0' }}>
          <Icon name="trophy" size={14} /> +{order.pointsEarned} PapaPoints earned · deposit hold of {money(order.depositHold)} released <Icon name="check" size={14} />
        </p>
      )}
